...
DefaultAssay(sobj_final) <- 'integrated.ADT'
...
sobj_final$new_diseasegroups <- factor(sobj_final$new_diseasegroups, levels = c('TNJDM', 'Active', 'Inactive', 'HC'))
...
DoHeatmap(subset(sobj_final, downsample = 100), features = genes, group.by = 'new_labels') +
  scale_fill_gradient2(low = 'blue', mid = 'white', high = 'red')
ggsave('rna.pdf', width = 6, height = 7)
...
donors <- sobj_final$study_id_visit
...
#Remove B cells mixed from both and HC from correlation
cell_prop_sample <- filter(cell_prop_sample, label != 'B_naive4')
#cell_prop_sample <- filter(cell_prop_sample, label != 'B cells, patient_specific')
cell_prop_sample2 <- subset(cell_prop_sample, disease_group != "HC")


types <- c('vasglobal', 'vascutaneous', 'vasmuscle')
for_plot <- data.frame(matrix(ncol = 4, nrow = 0))
colnames(for_plot) <- c("prop_cor", 'prop_p', 'cell', 'disease measure')
for(n in types){
...
for_plot$Significant <- as.factor(ifelse(for_plot$prop_p < 0.05, "TRUE", "FALSE"))
...
plot1 <- ggplot(cell_prop_sample3, aes(label, percent, fill = disease_group)) +
  ...
  scale_y_log10() +
  scale_fill_manual(values = c('HC' = 'skyblue',
                               'Inactive' = 'gold1',
...
plot_grid(plot2, plot1,
          align = 'hv', axis = 'l', nrow = 2, ncol = 1, rel_heights = c(1,1))
