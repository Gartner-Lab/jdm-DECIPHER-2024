...
# load the data
OUT_DIR="/krummellab/data1/erflynn/premier/jdm/data/jdm/wnn_v2/reclus/"
setwd(OUT_DIR)

load("wnn_reclus.RData")
head(sobj@meta.data)
...
## labels already added --> transfer
sobj@meta.data$wnn_cluster_label=NULL
sobj@meta.data$azimuth_cluster_label=NULL

sobj = transfer_annot(sobj, "wsnn_leiden_reclus_res.1.4", "wnn_cluster_label")
clus_map = sobj@meta.data %>% distinct(wsnn_leiden_reclus_res.1.4, wnn_cluster_label)
...
sobj = transfer_annot(sobj, "wsnn_leiden_reclus_res.1.4", "azimuth_cluster_label")
...
p1 = DimPlot(sobj, group.by="wsnn_leiden_reclus_res.1.4",reduction="wnn.umap", cols=unname(col_vec), label=T,
...
# load the RNA markers

rna_markers = do.call(rbind, lapply(1:29, function(i){
  m = readRDS(sprintf("markers/RNA_wsnn_leiden_reclus_res.1.4_markers_%s.RDS", i))
  m %>% as_tibble(rownames="gene") %>% mutate(cluster=i)
}))

# make a dotplot
gen_dotplot(sobj, "RNA", rna_markers)
ggsave("rna_dotplot.pdf", height=18, width=12)

rna_markers %>%
  group_by(cluster) %>%
  left_join(clus_map %>% mutate(wsnn_leiden_reclus_res.1.4=as.numeric(as.character(wsnn_leiden_reclus_res.1.4))),
            by=c("cluster"="wsnn_leiden_reclus_res.1.4")) %>%
  arrange(wnn_cluster_label, p_val_adj, desc(abs(avg_log2FC))) %>%
  slice_head(n=50) %>%
...
wnn_markers_adt %>%
  ...
  group_by(cluster) %>%
  left_join(clus_map %>% 
              mutate(wsnn_leiden_reclus_res.1.4=as.numeric(as.character(wsnn_leiden_reclus_res.1.4))),
            by=c("cluster"="wsnn_leiden_reclus_res.1.4")) %>%
  arrange(wnn_cluster_label, p_val_adj, desc(abs(avg_log2FC))) %>%
  slice_head(n=50) %>% 
...
ggsave(sprintf("%s/adt_dotplot.pdf", OUT_DIR), width=11, height=17)
