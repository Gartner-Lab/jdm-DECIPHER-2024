...
```{r load libraries}
#Script to run network GEP/module annotation section of the DECIPHER pipeline on JDM cross-long study data (post-nmf run and network construction)
library(Seurat)
library(ggplot2)
library(svglite)
library(rlist)
...
pdf(file = 'DECIPHER_outputs/gene_loading_similarity_network.pdf', height = 10, width = 10)
...
gobp_sig_list = readRDS('DECIPHER_outputs/network_gobp_fgsea_sig.rds')
program_markers = readRDS('DECIPHER_outputs/program_marker_gene_lists.rds')
GO_BP_fgsea_res = readRDS('DECIPHER_outputs/GOBP_fgsea_res.rds')
```
