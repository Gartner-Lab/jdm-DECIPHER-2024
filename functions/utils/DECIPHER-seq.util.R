...
cell_type_cols=c('#33A02B','#875890','#B2DF8A','#A6CEE3','#F8766D','#1F78B4')
network_module_cols = c("#1B9E77", "#E6AB02", "#D95F02", "#7570B3", "#E7298A", "#66A61E", '#ffcc12', "#E41A1C", "#377EB8", '#E68D1A','#CEA6A6' )
#adjusted color palettes based on n cell types and n modules
...
plot_hists = function(phylo_trees, suggested_thresholds = NULL, thresh.use = NULL){
  ...
  if (is.null(thresh.use)){
    thresh.use = round(min(suggested_thresholds), 1)
  }
...
plot_fgsea <- function(Network, fgsea_res, gene_set, fdr_max = 1e-1, fdr_min = 1e-10){
  ...
  plot(Network$filtered_network, layout = Network$filtered_network_coords, 
       vertex.color=colorRampPalette(c("grey", "red"))(101)[cut(-log10(fdr_vect), breaks = c(0, seq(-log10(fdr_max), -log10(fdr_min), length.out = 99), max(-log10(fdr_vect))), include.lowest = T, right = F)],
