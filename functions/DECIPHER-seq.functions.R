...
Get_enrichment_pvals <- function(sets_to_test, Network, nreps = 10000){
  ...
  # only test for enrichment for gene sets with at least 3  significantly enriched nodes (fdr < 0.01) #customized for jdm
  fgsea_test = subset(fgsea_test, pathway %in% names(which(table(subset(fgsea_test, fdr < 0.01&NES>0)$pathway)>=3)))
  fgsea_test$node_id = paste0(fgsea_test$Type, '.', fgsea_test$Program)
...
# Measure effect size and p values for association with metadata features #### #anova for categorical features > 2 groups
Calculate_metadata_associations <- function(Network, Expression_score, metadata, feature.to.test = NULL, type = c("binary", "continuous", 'categorical>2'), filtered = TRUE){
  ...
  H.score.comb$feature.to.test = plyr::mapvalues(rownames(H.score.comb), from = sc_metadata$Sample, to = as.character(sc_metadata[,feature.to.test]), warn_missing = F)
  ...
  if(filtered == TRUE){
    ...
    res = data.frame(matrix(nrow = 3, ncol = ncol(Expression_score)))
    colnames(res) = colnames(Expression_score)}
  ...
  if (type == "binary"){
    ...
    if(filtered == TRUE){
      for (i in colnames(res)){
        res[c("effect_size"),i] = as.numeric(t_test(H.score.comb, as.formula(paste0(i, "~", "feature.to.test")),
                                                            ref.group = tail(test_conditions, n=1), detailed = T)$estimate)
        res[c("effect_size_sign", "p_value"),i] = as.numeric(t_test(data = H.score.comb, as.formula(paste0(i, "~", "feature.to.test")),
                                                                         ref.group = tail(test_conditions, n = 1), detailed = T)[,c("estimate", "p")])}
    res["effect_size_sign",][res["effect_size_sign",]>0] = 1
    res["effect_size_sign",][res["effect_size_sign",]<0] = -1
    } else{
      for (i in colnames(res)){
        res[c("effect_size"),i] = as.numeric(t_test(H.score.comb, as.formula(paste0(i, "~", "feature.to.test")),
                                                    ref.group = 'JDM', detailed = T)$estimate)
        res[c("effect_size_sign", "p_value"),i] = as.numeric(t_test(data = H.score.comb, as.formula(paste0(i, "~", "feature.to.test")),
                                                                    ref.group = 'JDM', detailed = T)[,c("estimate", "p")])}
      res["effect_size_sign",][res["effect_size_sign",]>0] = 1
      res["effect_size_sign",][res["effect_size_sign",]<0] = -1
    }
    ...
    if(filtered == TRUE){
      for (i in colnames(res)){
        res[c("effect_size-eta^2"),i] = as.numeric(rstatix::anova_test(H.score.comb, as.formula(paste0(i, "~", "feature.to.test")))$ges)
        res[c("p_value"),i] = as.numeric(rstatix::anova_test(H.score.comb, as.formula(paste0(i, "~", "feature.to.test")))$p)
        pairwise_comp[[i]] <- tukey_hsd(H.score.comb, as.formula(paste0(i, "~", "feature.to.test")), p.adjust.method = "BH")
      }
